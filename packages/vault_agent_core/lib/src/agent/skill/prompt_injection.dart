...
SystemPromptPart? buildSkillSystemPrompt(
  ...
  final forceActiveSkills = skills
      .where((skill) => skill.forceActivate)
      .toList();
  final optionalSkills = skills.where((skill) => !skill.forceActivate).toList();
  final activeSkillNames = {
    ...(state.activeSkills ?? []),
    ...forceActiveSkills.map((skill) => skill.name),
  }.toList();
  ...
  if (optionalSkills.isNotEmpty) {
    ...
    if (forceActiveSkills.isNotEmpty) {
      ...
      for (final skill in forceActiveSkills) {
    ...
    for (final skill in optionalSkills) {
      final status = activeSkillNames.contains(skill.name)
          ? '🟢 [ACTIVE]'
          : '⚪ [INACTIVE]';
      buffer.writeln('- **${skill.name}** $status: ${skill.description}');
    }
    buffer.writeln();
  ...
  if (activeSkillNames.isNotEmpty) {
    ...
    final activeSkills = skills.where(
      (skill) => activeSkillNames.contains(skill.name),
    );
...
List<DirectorySkillMetadata> collectExplicitDirectorySkillMentions(
  ...
  for (final skill in skills) {
    nameCounts.update(skill.name, (count) => count + 1, ifAbsent: () => 1);
  }
  ...
  for (final skill in skills) {
    final path = _normalizePath(skill.pathToSkillMd);
    if (mentionedNames.contains(skill.name) &&
        (nameCounts[skill.name] ?? 0) == 1 &&
        selectedPaths.add(path)) {
      selected.add(skill);
    }
  }
  ...
  for (final message in messages) {
    ...
    for (final part in message.contents) {
      ...
      for (final skill in skills) {
        final path = _normalizePath(skill.pathToSkillMd);
        if ((nameCounts[skill.name] ?? 0) == 1 &&
            _textContainsSkillName(part.text, skill.name) &&
            selectedPaths.add(path)) {
          selected.add(skill);
        }
      }
...
Future<DirectorySkillInjections> buildDirectorySkillInjections(
  ...
  for (final skill in mentionedSkills) {
    try {
      ...
      warnings.add(
        'Failed to load skill ${skill.name} at '
        '${skill.pathToSkillMd}: $error',
      );
    }
  }
