...
void main() {
  ...
  test(
    'fullModelMessage does not replay text already flushed before tools',
    () async {
      final mapper = AgentStreamMapper(
        runningBackgroundJobs: () => const <BackgroundToolJob>[],
      );
  ...
  test(
    'stream mapper emits thought deltas without duplicating full thought',
    () async {
      final mapper = AgentStreamMapper(
        runningBackgroundJobs: () => const <BackgroundToolJob>[],
      );
      final buffer = StringBuffer();
      final first = await mapper
          .map(
            StreamingEvent(
              eventType: StreamingEventType.modelChunkMessage,
              data: ModelMessage(model: 'm', thought: '先想'),
            ),
            buffer,
          )
          .toList();
      final second = await mapper
          .map(
            StreamingEvent(
              eventType: StreamingEventType.fullModelMessage,
              data: ModelMessage(
      ...
      expect(first.whereType<AgentUiAssistantDelta>().single.thought, '先想');
      expect(
        second.whereType<AgentUiAssistantDelta>().where(
          (event) => (event.thought ?? '').isNotEmpty,
        ),
        isEmpty,
      );
      expect(second.whereType<AgentUiAssistantDelta>().single.text, '好的');
    },
  );

  test(
    'fullModelMessage still shows text when nothing streamed this turn',
    () async {
      final mapper = AgentStreamMapper(
        runningBackgroundJobs: () => const <BackgroundToolJob>[],
      );
      ...
      expect(events.whereType<AgentUiAssistantDelta>().single.text, '完成了');
    },
  );
