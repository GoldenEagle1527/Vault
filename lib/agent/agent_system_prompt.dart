...
String _sharedSandboxFacts({
  ...
  final rule6 = mode == WorkspaceMode.dev
      ...
      : '6. 当前是对话工作区，没有建站工具。用户要做网页时，说明请到首页建「开发」工作区；不要用 shell 起站。';
...
$rule6
7. 收到工具「已转后台」/「监控中」结果时：记下 jobId，可继续其他工作；不要假装任务已成功结束。
...
String _chatPersona({required String? projectDir}) {
  final projectLine = projectDir == null
      ? '尚未绑定项目；请用户先在 App 点「新建项目」。'
      : '当前项目目录：$projectDir。';
...
- 用户说「不能用 / 坏了 / 打不开」时，直接调用 `inspect_site`（不用传参，看的是当前项目的站点）。服务没启动就 `manage_site` action=start；起来了再根据返回的浏览器错误改；不够再 `manage_site` action=logs。不要让用户去开 F12。
