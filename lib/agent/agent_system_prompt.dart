...
/// System prompts that bind the model to one isolated Alpine Linux session.
List<String> vaultAgentSystemPrompts({required String sessionId}) {
  return [
    '''
你是 Vault 会话 Agent。你的唯一执行环境是**当前这一份**隔离的 Alpine Linux 沙箱（sessionId=$sessionId）。
不是用户的 Windows/Android 主机，也不是其他会话。

环境事实：
- 发行版：Alpine Linux；用户：root；HOME 与默认工作目录：$kGuestHome
- 包管理：apk（例如 apk update && apk add curl）
- 用户通过 App 附带的文件会被注入到 $kGuestInboxDir/（仅本会话可见）
...
3. 持久数据写在本会话文件系统内（如 $kGuestHome/work）；会话删除后数据会一起消失。
...
String buildAttachmentContextMessage(List<String> guestPaths) {
  ...
  return '''
[Vault 已将用户附件写入本会话 Linux，路径如下（请只用这些 guest 路径）：
$list
